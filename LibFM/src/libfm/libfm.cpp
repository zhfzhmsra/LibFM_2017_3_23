...
#include "tools/convert_zhfzhr.h"
...
int main(int argc, char **argv) { 
	...
	string parameterStr = R"(E:\Users\v-fuz\Codes\Tools\LibFM\Debug\LibFM.exe )"
		R"(-train \\msra-irm104\E$\users\v-fuz\tmp\checkin.rpt.filter.context.train.libfm )"
		R"(-test \\msra-irm104\E$\users\v-fuz\tmp\checkin.rpt.filter.context.test.libfm )"
		R"(-out \\msra-irm104\E$\users\v-fuz\tmp\checkin.rpt.filter.context.predict.libfm )"
		R"(-task r -dim 1,1,8 -method als -learn_rate 0.1 -iter 2)";

	////string parameterStr = R"(E:\Users\v-fuz\Codes\Tools\LibFM\Debug\LibFM.exe )"
	////	R"(-train \\mlsdata\e$\Users\v-fuz\Dataset\FlatFile\MyWork\Book_Rec\MovieLens\ml-1m\ml-1m\libfm_train.csv )"
	////	R"(-test \\mlsdata\e$\Users\v-fuz\Dataset\FlatFile\MyWork\Book_Rec\MovieLens\ml-1m\ml-1m\libfm_test.csv )"
	////	R"(-out \\mlsdata\e$\Users\v-fuz\Dataset\FlatFile\MyWork\Book_Rec\MovieLens\ml-1m\ml-1m\libfm_predict.csv )"
	////	R"(-task c -dim 1,1,150 -method als -learn_rate 0.05 -iter 30)";


	vector<string> strings = split(parameterStr, " ");
	
	argc = strings.size();
	argv = new char*[strings.size()];
	...
	clock_t end = clock();
	double elapsed_secs = double(end - begin) / (CLOCKS_PER_SEC * 60.0 * 60.0);

	std::cout << "Total time used: " << elapsed_secs << " hours" << endl;
}
